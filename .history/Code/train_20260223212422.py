...
def build_dataset(cfg: dict) -> tuple[np.ndarray, np.ndarray]:
    ...
    data_root = (THESIS_ROOT / cfg["data"]["processed_data_root"]).resolve()
    ...
    if not all_X:
        raise RuntimeError("No training data found! Check processed_data_root in config.")
...
def main():
    ...
    config_path = (CODE_DIR / args.config).resolve()
    ...
    # Save normalizer
    ckpt_dir = (CODE_DIR / cfg["training"]["checkpoint_dir"]).resolve()
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    normalizer.save(ckpt_dir / "normalizer.npz")
    print(f"  Saved normalizer → {ckpt_dir / 'normalizer.npz'}\n")
    ...
    counts    = np.bincount(y_all[trn_idx], minlength=cfg["model"]["num_classes"]).astype(float)
    ...
    print(f"{'Epoch':>6}  {'Trn Loss':>9}  {'Trn Acc':>8}  {'Val Loss':>9}  {'Val Acc':>8}  {'LR':>10}")
    print("-" * 60)

    for epoch in range(1, cfg["training"]["epochs"] + 1):
        t0 = time.time()
        trn_loss, trn_acc = train_one_epoch(model, trn_loader, optimizer, criterion, device)
    ...
    class_acc = per_class_accuracy(model, val_loader, device, cfg["model"]["num_classes"])
    print("\n  Per-class accuracy (validation):")
    for cls, acc in class_acc.items():
        print(f"    {cls:30s}: {acc:.4%}")

    # Save training history & results
    save_json(history,   ckpt_dir / "training_history.json")
    save_json(class_acc, ckpt_dir / "class_accuracy.json")
    save_json({"input_size": F, "num_classes": cfg["model"]["num_classes"],
               "model_type": cfg["model"]["type"]},
              ckpt_dir / "model_meta.json")
