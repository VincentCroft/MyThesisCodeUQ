...
def polar_to_rect(mag: np.ndarray, ang_deg: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
...
def sliding_windows(
    ...
    if not xs:
        return np.empty((0, window, features.shape[1]), dtype=np.float32), np.empty(0, dtype=np.int64)
