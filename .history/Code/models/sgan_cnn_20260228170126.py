...
class SGANGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.merge = nn.Sequential(
            nn.Conv1d(base_ch * 3, base_ch * 2, kernel_size, padding=kernel_size // 2, bias=False),
...
class SGANCNNTrainer(nn.Module):
    ...
    def generator_loss(
        ...
        l_fm = F.mse_loss(feat_fake.mean(0), feat_real.mean(0)
                          ) + F.mse_loss(feat_fake.std(0), feat_real.std(0))
