...
def _plotly_iframe(fig, height: int = 350, key: str = "", legend_width: int = 0) -> None:
    ...
    * autosize=True so the chart fills the iframe width on first paint.
...
(function() {{
    ...
    // Reserve right-side margin so legend text won't be clipped.
    // (Some plots place legend in the right margin area.)
        if (legendW > 0) {{
            if (!fig.layout.margin) {{ fig.layout.margin = {{}}; }}
            fig.layout.margin.r = Math.max(fig.layout.margin.r || 0, legendW);
        }}
  ...
  // Re-layout on fullscreen change so chart fills the screen properly
  document.addEventListener('fullscreenchange', function() {{
    var el = document.getElementById('{ uid }');
    if (document.fullscreenElement === el) {{
      Plotly.relayout(el, {{ width: screen.width, height: screen.height }});
    }} else {{
      Plotly.relayout(el, {{ width: null, height: { height } }});
