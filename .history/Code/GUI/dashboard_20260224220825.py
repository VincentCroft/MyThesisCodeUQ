...
(function () {
  ...
  function setupRadio(rg) {
    ...
    function movePillTo(fromEl, toEl, animate) {
      ...
      if (tRect.width === 0) return;
      pill.style.transition = animate
        ? 'left .26s cubic-bezier(.25,.8,.25,1),top .26s cubic-bezier(.25,.8,.25,1),' +
          'width .26s cubic-bezier(.25,.8,.25,1),height .26s cubic-bezier(.25,.8,.25,1)'
        : 'none';
      pill.style.left   = (tRect.left - cRect.left) + 'px';
      pill.style.top    = (tRect.top  - cRect.top)  + 'px';
      pill.style.width  = tRect.width  + 'px';
    ...
    // ── Path A: DOM stays, only aria-checked changes (in-place Streamlit update) ──
    // Save the current label on mousedown (BEFORE aria-checked flips)
    rg.addEventListener('mousedown', function (e) {
      var item = e.target.closest('[data-baseweb="radio"]');
      if (item && idx !== -1) win._stRadioPrev[idx] = checkedLabel();
    });

    // React to aria-checked change → animate pill
    ...
    // ── Path B: Full DOM rebuild (Streamlit rerun creates a new rg element) ──
