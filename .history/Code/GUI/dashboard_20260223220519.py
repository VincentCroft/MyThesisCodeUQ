...
# ── 统一路径配置（修改 configs/paths.py 即可调整所有路径）──
from configs.paths import (
    PROCESSED_DATA_ROOT,
    CKPT_DIR,
    TRAINING_HISTORY_JSON  as HISTORY_PATH,
    CLASS_ACC_JSON,
    MODEL_META_JSON        as META_PATH,
    NORMALIZER_NPZ         as NORMALIZER,
    BEST_MODEL_PT          as BEST_MODEL,
    CONFUSION_MATRIX_JSON  as CM_PATH,
    TSNE_EMBEDDINGS_JSON   as TSNE_PATH,
...
CM_PATH   = CKPT_DIR / "confusion_matrix.json"
TSNE_PATH = CKPT_DIR / "tsne_embeddings.json"
...
with st.sidebar:
    ...
    if model_exists():
        ...
        if META_PATH.exists():
            ...
            st.caption(
                f"Type: **{meta.get('model_type','?')}**  |  Classes: **{meta.get('num_classes','?')}**"
            )
    else:
...
if page == "🏠  Home":
    ...
    with c1:
        st.markdown(
            '<div class="section-header">🗂  支持故障类型</div>', unsafe_allow_html=True
        )
        st.markdown("")
    ...
    with c2:
        st.markdown(
            '<div class="section-header">📐  输入特征说明</div>', unsafe_allow_html=True
        )
    ...
    st.markdown("---")
    st.markdown(
        '<div class="section-header">📊  训练历史</div>', unsafe_allow_html=True
    )
    hist = load_history()
    if hist:
        try:
...
# ═══════════════════════════════════════════════════════════
    ...
    with col_btn:
        start_btn = st.button(
            "▶️  开始训练 Start Training", use_container_width=True, type="primary"
        )
    with col_info:
        if model_exists():
...
# ═══════════════════════════════════════════════════════════
    ...
    st.title("📈 模型分析 / Model Analysis")
    st.markdown("训练完成后，查看拟合曲线、混淆矩阵、分类报告与特征聚落图。")
    st.markdown("---")
    ...
    try:
        ...
        _PLOTLY = False
        st.warning("⚠️  plotly 未安装，图表将以简化形式显示。")
    ...
    with tab_curve:
        st.markdown('<div class="section-header">训练 / 验证拟合曲线</div>',
                    unsafe_allow_html=True)
        hist = load_history()
        if not hist:
            st.info("尚无训练历史，请先训练模型。")
        else:
            epochs = list(range(1, len(hist["val_acc"]) + 1))
            ...
            m1.metric("最佳验证准确率", f"{best_acc:.2%}")
            m2.metric("最佳 Epoch", best_ep)
            m3.metric("最终训练Loss", f"{hist['train_loss'][-1]:.4f}")
            m4.metric("最终验证Loss", f"{hist['val_loss'][-1]:.4f}")
            ...
            if _PLOTLY:
                ...
                fig_acc.add_trace(go.Scatter(
                    x=epochs, y=[v * 100 for v in hist["train_acc"]],
                    name="Train Acc (%)", mode="lines+markers",
                    line=dict(color="#38bdf8", width=2),
                    marker=dict(size=4),
                ))
                fig_acc.add_trace(go.Scatter(
                    x=epochs, y=[v * 100 for v in hist["val_acc"]],
                    name="Val Acc (%)", mode="lines+markers",
                    line=dict(color="#34d399", width=2),
                    marker=dict(size=4),
                ))
                fig_acc.add_vline(
                    x=best_ep, line_dash="dot", line_color="#fbbf24",
            ...
            st.markdown("---")
            st.markdown('<div class="section-header">过拟合分析</div>',
                        unsafe_allow_html=True)
            gap = [
                round((ta - va) * 100, 2)
            ...
            if _PLOTLY:
                ...
                fig_gap.add_trace(go.Scatter(
                    x=epochs, y=gap, name="Train-Val Gap (%)",
                    fill="tozeroy",
                    line=dict(color="#f472b6", width=2),
                ))
                fig_gap.add_hline(y=5,  line_dash="dot", line_color="#fbbf24",
            ...
            final_gap = gap[-1]
            if final_gap < 3:
                st.success(f"✅  最终 Train-Val Gap = {final_gap:.2f}%，拟合良好。")
            elif final_gap < 8:
                st.warning(f"⚠️  最终 Train-Val Gap = {final_gap:.2f}%，轻度过拟合，可适当增大 dropout。")
            else:
                st.error(f"❌  最终 Train-Val Gap = {final_gap:.2f}%，过拟合较严重，建议增大正则化或减少模型复杂度。")

    # ── Tab 2: Confusion Matrix ─────────────────────────────
    with tab_cm:
        st.markdown('<div class="section-header">混淆矩阵 Confusion Matrix</div>',
                    unsafe_allow_html=True)

        if not CM_PATH.exists():
            st.info("⏳  混淆矩阵数据尚未生成，请重新训练一次模型（训练结束时自动生成）。")
        else:
            ...
            st.metric("整体准确率 Overall Accuracy", f"{oa:.2%}")
            st.markdown("")

            view_mode = st.radio(
                "显示模式", ["原始计数 Raw Counts", "归一化 Normalised (Recall)"],
                horizontal=True,
            )
            use_norm = "归一化" in view_mode
            ...
            if _PLOTLY:
                ...
                fig_cm.update_layout(
                    title="Confusion Matrix" + (" (Normalised)" if use_norm else " (Raw Counts)"),
                    paper_bgcolor="#0f172a",
            ...
            # Per-class table
            st.markdown("---")
            st.markdown('<div class="section-header">各类别指标</div>',
                        unsafe_allow_html=True)
            rows = []
    ...
    with tab_report:
        st.markdown('<div class="section-header">分类报告 Classification Report</div>',
                    unsafe_allow_html=True)

        if not CM_PATH.exists():
            st.info("⏳  请先完成训练，自动生成分类报告数据。")
        else:
            cm_data = json.loads(CM_PATH.read_text())
            cls_names = cm_data["class_names"]
            oa = cm_data["overall_accuracy"]

            # Radar chart: F1 / Precision / Recall per class
            if _PLOTLY:
                ...
                for cls in cls_names:
                    vals = [
                        ...
                        per[cls]["recall"],
                        per[cls]["f1"],
                        per[cls]["precision"],   # close polygon
                    ]
            ...
            rows.append({
                "Class": "macro avg",
                "Precision": round(np.mean([cm_data["per_class"][c]["precision"] for c in cls_names]), 4),
                "Recall":    round(np.mean([cm_data["per_class"][c]["recall"]    for c in cls_names]), 4),
                "F1-Score":  round(np.mean([cm_data["per_class"][c]["f1"]        for c in cls_names]), 4),
                "Support":   sum(cm_data["per_class"][c]["support"] for c in cls_names),
            })
            rows.append({
            ...
            st.markdown("---")
            st.markdown('<div class="section-header">指标汇总表</div>',
                        unsafe_allow_html=True)
            st.dataframe(pd.DataFrame(rows), hide_index=True, use_container_width=True)

    # ── Tab 4: t-SNE Feature Cluster ───────────────────────
    with tab_tsne:
        st.markdown('<div class="section-header">特征聚落图 Feature Cluster (t-SNE)</div>',
                    unsafe_allow_html=True)
        st.caption("使用 t-SNE 将模型倒数第二层的高维特征投影到 2D 平面，观察各故障类别的可分离性。")

        if not TSNE_PATH.exists():
            st.info("⏳  t-SNE 嵌入数据尚未生成，请重新训练一次模型（训练结束时自动生成）。")
        else:
            ...
            if _PLOTLY:
                ...
                fig_tsne.update_layout(
                    ...
                    paper_bgcolor="#0f172a", plot_bgcolor="#0f172a",
                    font=dict(color="#94a3b8"),
                    xaxis=dict(title="t-SNE Dim 1", gridcolor="#1e293b",
                               zeroline=False, showticklabels=False),
                    yaxis=dict(title="t-SNE Dim 2", gridcolor="#1e293b",
                               zeroline=False, showticklabels=False),
                    legend=dict(bgcolor="#1e293b", bordercolor="#334155",
                                itemsizing="constant"),
                    margin=dict(l=10, r=10, t=50, b=10),
                ...
                # Class distribution in embedded space
                st.markdown("---")
                st.markdown('<div class="section-header">各类别样本分布</div>',
                            unsafe_allow_html=True)
                ...
                fig_dist = go.Figure(go.Bar(
                    x=dist["Class"],
                    y=dist["Count"],
                    marker_color=[CLASS_COLORS.get(c, "#94a3b8") for c in dist["Class"]],
                    text=dist["Count"],
                    textposition="outside",
                ))
                ...
                st.scatter_chart(df_tsne[["x", "y", "label_name"]],
                                 x="x", y="y", color="label_name")
...
# ═══════════════════════════════════════════════════════════
    ...
    if uploaded is not None:
        ...
        st.markdown(
            '<div class="section-header">📋  数据预览 Data Preview</div>',
            unsafe_allow_html=True,
        )
        ...
        # ── 3. Results ──
        st.markdown(
            '<div class="section-header">🎯  推理结果 Inference Results</div>',
            unsafe_allow_html=True,
        )
        st.markdown("")
        ...
        st.markdown(
            '<div class="section-header">📈  逐窗口预测分布 Per-Window Distribution</div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown("---")
        st.markdown(
            '<div class="section-header">📊  概率汇总 Probability Summary</div>',
            unsafe_allow_html=True,
        )
