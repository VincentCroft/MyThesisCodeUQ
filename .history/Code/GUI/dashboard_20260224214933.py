...
(function () {
  ...
  function setupRadio(rg) {
    ...
    function upd(anim) {
      // Streamlit radio uses [role="radiogroup"] -> [data-baseweb="radio"]
      // The checked one has aria-checked="true" OR a child input is checked.
      // We look for aria-checked on the radio container div.
      var a = rg.querySelector('[aria-checked="true"]');
      if (a) movePill(pill, a, rg, anim);
    }

    // 1. Observe attribute changes (for simple clicks where DOM is stable)
    new MutationObserver(function () { upd(true); })
      .observe(rg, { attributes: true, subtree: true, attributeFilter: ['aria-checked'] });

    // 2. Observe child list changes (in case Streamlit rebuilds the radio buttons entirely)
    // If the radio buttons are re-rendered, we need to re-find the active one and animate to it.
    new MutationObserver(function () {
      // Check if our pill was wiped out by a full re-render
      if (!rg.querySelector('.sp-pill')) {
        pill = createPill(rg); // Re-create pill if missing
        rg.classList.add('sp-ready'); // Ensure class acts immediately
      }
      upd(true);
    }).observe(rg, { childList: true, subtree: true });

    // Initial setup with polling for layout readiness
    function tryInit(attempts) {
      ...
      pill.style.transition = 'none'; // No animation on initial load
      ...
      // Force a reflow before adding transition back (if we wanted to, but here we just set ready)
      requestAnimationFrame(function () { 
