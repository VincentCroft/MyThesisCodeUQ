...
def _show_plotly(fig, height: int = 400, key: str = "") -> None:
    ...
    html_str = fig.to_html(
        full_html=True,
        include_plotlyjs="cdn",   # use CDN to keep payload small
        config={
            "displayModeBar": True,
            "displaylogo": False,
    ...
    # Stretch iframe to fill column width; add a tiny padding so scrollbar
    # never appears.
    _components.html(html_str, height=height + 20, scrolling=False)
    ...
    with tab_report:
        ...
        if not CM_PATH.exists():
            ...
            if _PLOTLY:
                ...
                st.plotly_chart(
                    radar_fig, use_container_width=True, key=f"fig_radar_{_sel_run}"
                )
                ...
                st.plotly_chart(
                    fig_f1, use_container_width=True, key=f"fig_f1_{_sel_run}"
                )
    ...
    with tab_tsne:
        ...
        if not TSNE_PATH.exists():
            ...
            if _PLOTLY:
                ...
                st.plotly_chart(
                    fig_tsne, use_container_width=True, key=f"fig_tsne_{_sel_run}"
                )
                ...
                st.plotly_chart(
                    fig_dist, use_container_width=True, key=f"fig_dist_{_sel_run}"
                )
            else:
                st.scatter_chart(
        ...
        for r in results_cache:
            ...
            with st.expander(
                ...
                try:
                    ...
                    st.plotly_chart(
                        fig2, use_container_width=True, key=f"fig_infer_{fname}"
                    )
                except ImportError:
