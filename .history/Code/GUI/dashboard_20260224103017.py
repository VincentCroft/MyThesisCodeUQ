...
def _plotly_iframe(fig, height: int = 350, key: str = "") -> None:
    ...
    The iframe height is set to `height` px. The body has no overflow clipping,
    so legend text that extends slightly beyond the paper area is never cut off.
    Legend: single-click = toggle, double-click = isolate/restore all.
    Modebar: no Plotly logo, no "Edit chart" link, custom fullscreen button added.
    """
...
<style>
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  /* No overflow:hidden — legend text must never be clipped */
  html {{ width:100%; height:100%; background:#0f172a; }}
  body {{ width:100%; min-height:{ height }px; background:#0f172a; }}
...
(function() {{
  ...
  document.addEventListener('fullscreenchange', function() {{
    ...
    # scrolling=True ensures any minor overflow is accessible rather than hidden
    _components.html(html, height=height + 20, scrolling=False)
            ...
            if _PLOTLY:
                ...
                radar_fig.update_layout(
                    polar=dict(
                        ...
                        # shrink polar domain so legend below has room
                    ...
                    legend=dict(
                        ...
                        x=0.5,
                        xanchor="center",
                        y=0.04,          # sits in the space below the polar domain
                        yanchor="bottom",
                    ),
                    title="Per-Class Precision / Recall / F1 Radar",
                    margin=dict(l=40, r=40, t=55, b=10),
                    height=430,
                    autosize=True,
                )
                _plotly_iframe(radar_fig, height=450, key="radar")
        ...
        if not TSNE_PATH.exists():
            ...
            if _PLOTLY:
                ...
                fig_tsne.update_layout(
                    ...
                    # ── legend at the bottom, horizontal, fully visible ──
                    legend=dict(
                        ...
                        x=0.5,
                        xanchor="center",
                        y=-0.08,  # just below the plot area
                        yanchor="top",
                    ),
                    margin=dict(l=50, r=20, t=50, b=80),  # b=80 makes room for legend
                    height=560,
                    autosize=True,
                )
                _plotly_iframe(fig_tsne, height=576, key="tsne")
