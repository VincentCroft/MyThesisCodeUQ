...
def render_plotly(fig, height: int = 400, key: str = "") -> None:
    ...
    # 2. Legend placement
    #    • Cartesian charts  → horizontal strip BELOW the plot (never clips)
    #    • Polar/radar chart → vertical strip to the RIGHT, with enough
    #      right-margin so text is never cropped
    ...
    if _has_polar:
        ...
        _mr = 180  # wide enough for longest class name
        _legend_b_extra = 0
    else:
        lay.legend = dict(
            orientation="h",
        ...
        _mr = 20
        _legend_b_extra = 70  # extra pixels below x-axis for h-legend
    ...
    # 4. Always autosize — Plotly fills whatever space the container gives
    lay.autosize = True
    lay.width = None
    lay.height = height + _legend_b_extra  # plot area height

    fig_json = _pio.to_json(fig2, validate=False)

    # iframe height = chart height + modebar (30) + small safety pad (6)
    iframe_h = height + _legend_b_extra + 36
