...
# ════════════════════════════════════════════════════════════
    ...
    try:
        ...
        def render_plotly_iframe(fig, *, height: int, key: str):
                """Render Plotly charts inside an iframe (components.html) to avoid legend flicker.

                Key points:
                - iframe is isolated document; Streamlit VDOM diffing won't interfere.
                - plotly.js loaded from CDN (not inlined).
                - Unique div_id per chart avoids collisions on pages with many figures.
                - Dark theme background injected to match #0f172a with no white border.
                - Adds a fullscreen button while keeping the inline chart size unchanged.
                - Removes Plotly logo / "Produced with Plotly.js".
                """

                import streamlit.components.v1 as components

                # Fullscreen state
                fs_state_key = "_plotly_fullscreen"
                st.session_state.setdefault(fs_state_key, None)

                cols = st.columns([1, 11])
                with cols[0]:
                        if st.button("⛶", key=f"fs_btn_{key}", help="Fullscreen"):
                                st.session_state[fs_state_key] = key

                div_id = f"plotly-{key}-{uuid.uuid4().hex}"

                # Hide logo and keep interactions; legend single/multi selection is Plotly default.
...
<html>
    <head>
        ...
        <style>
            ...
            #wrap {{ width:100%; height:{height}px; background:#0f172a; }}
            #{div_id} {{ width:100%; height:100%; background:#0f172a; }}
            .js-plotly-plot .plotly, .js-plotly-plot .plotly div {{ background:#0f172a !important; }}
        </style>
    </head>
    <body>
        <div id=\"wrap\"><div id=\"{div_id}\"></div></div>
        <script>
            const fig = {fig_json};
            const config = {json.dumps(plotly_config)};
            fig.layout = fig.layout || {{}};
            fig.layout.paper_bgcolor = fig.layout.paper_bgcolor || '#0f172a';
            fig.layout.plot_bgcolor  = fig.layout.plot_bgcolor  || '#0f172a';
            ...
            ro.observe(document.body);
                ...
                if st.session_state.get(fs_state_key) == key:
                        fs_cols = st.columns([1, 11])
                        with fs_cols[0]:
                                if st.button("✕", key=f"fs_close_{key}", help="Close fullscreen"):
                                        st.session_state[fs_state_key] = None
                                        st.rerun()
    ...
    with tab_curve:
        ...
        if not hist:
            ...
            if _PLOTLY:
                ...
                with col_a:
                    render_plotly_iframe(fig_acc, height=300, key="analysis_acc")
                with col_b:
                    render_plotly_iframe(fig_loss, height=300, key="analysis_loss")
            else:
                st.line_chart(
            ...
            if _PLOTLY:
                ...
                render_plotly_iframe(fig_gap, height=250, key="analysis_gap")
    ...
    with tab_cm:
        ...
        if not CM_PATH.exists():
            ...
            if _PLOTLY:
                ...
                render_plotly_iframe(fig_cm, height=420, key="analysis_cm")
            else:
    ...
    with tab_report:
        ...
        if not CM_PATH.exists():
            ...
            if _PLOTLY:
                ...
                render_plotly_iframe(radar_fig, height=380, key="analysis_radar")
                ...
                render_plotly_iframe(fig_f1, height=280, key="analysis_f1")
    ...
    with tab_tsne:
        ...
        if not TSNE_PATH.exists():
            ...
            if _PLOTLY:
                ...
                render_plotly_iframe(fig_tsne, height=520, key="analysis_tsne")
                ...
                render_plotly_iframe(fig_dist, height=240, key="analysis_tsne_dist")
            else:
                st.scatter_chart(
