...
def _show_plotly(fig, height: int = 400, key: str = "") -> None:
    ...
    This bypasses Streamlit's component update mechanism entirely — the chart
    lives inside an <iframe> and is NEVER re-rendered on Streamlit reruns,
    eliminating all flicker caused by Python-side figure reconstruction.

    Legend interactions (single-click toggle, double-click isolate) are handled
    100% by the browser-side Plotly.js bundle embedded in the HTML.
    """
    # Give every chart a stable div id so Plotly.js can target it precisely
    ...
    # Inject a tiny style so the chart always fills 100 % of the iframe width
    # without any scrollbars appearing.
    inject = (
        "<style>body{margin:0;padding:0;overflow:hidden;background:#0f172a;}"
        f"#{div_id}{{width:100%!important;}}</style>"
    )
