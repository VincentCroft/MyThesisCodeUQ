...
def render_plotly(fig, height: int = 400, key: str = "") -> None:
    ...
    # Serialise figure to JSON (no inline JS library — load from CDN once)
...
(function(){{
  ...
  /* Force layout width = actual container pixel width so the chart
     fills the iframe edge-to-edge regardless of Plotly defaults.   */
  function containerW() {{
    return document.getElementById('chart-wrap').clientWidth || window.innerWidth;
  }}

  /* Override whatever width Plotly may have stored in the figure */
  fig.layout = fig.layout || {{}};
  fig.layout.width  = null;   // will be set right before newPlot
  ...
  /* Render: measure container first, pass width explicitly */
  fig.layout.width = containerW();
  Plotly.newPlot(divId, fig.data, fig.layout, config).then(function(){{
    normalW = containerW();
  }});

  /* Resize observer: keep chart full-width if the iframe itself resizes */
  ...
  /* Esc-key fullscreen exit safety net */
  function onFSChange() {{
    var inFS = !!(document.fullscreenElement
              || document.webkitFullscreenElement
              || document.mozFullScreenElement);
