...
(function () {
  ...
  // ─── Radio groups ────────────────────────────────────────
  var radioState = {}; // Store last known positions by index

  function setupRadio(rg) {
    var pill = createPill(rg);

    // Identify this radio group by its index on the page to persist state
    var diffs = Array.from(doc.querySelectorAll('[data-testid="stRadio"] > div:last-child'));
    var idx = diffs.indexOf(rg);
    ...
    function tryInit(attempts) {
      ...
      // Check if we have a stored previous state for this radio group
      if (radioState[idx]) {
        // Start from previous position
        pill.style.transition = 'none';
        pill.style.left   = radioState[idx].left + 'px';
        pill.style.top    = radioState[idx].top + 'px';
        pill.style.width  = radioState[idx].w + 'px';
        pill.style.height = radioState[idx].h + 'px';
        pill.style.opacity = '1';
