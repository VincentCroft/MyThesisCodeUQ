...
def _show_plotly(fig, height: int = 400, key: str = "") -> None:
    """Render a Plotly figure as a fully self-contained HTML iframe.

    The chart lives inside an <iframe> and is never re-rendered on Streamlit
    reruns, eliminating all flicker.  Legend interactions (single-click toggle,
    double-click isolate) and all Plotly animations are handled entirely by the
    browser-side Plotly.js bundle.
    """
    div_id = f"plt_{abs(hash(key)) % 10_000_000}"
    ...
    # ── Inject styles + a resize helper so the chart fills the iframe ──
    # The resize script runs once after Plotly finishes its first render,
    # which also re-triggers the entry animation correctly.
    inject = f"""
<style>
  html, body {{
    margin: 0; padding: 0;
...
<script>
  // Wait for Plotly to be available, then resize once to fill iframe width.
  // This preserves the draw-in animation because Plotly re-renders after resize.
  (function waitAndResize() {{
    if (typeof Plotly === "undefined") {{
      setTimeout(waitAndResize, 50);
      return;
    }}
    var el = document.getElementById("{div_id}");
    if (!el) {{ setTimeout(waitAndResize, 50); return; }}
    // Small delay so the initial animated draw completes first,
    // then snap to correct width without re-triggering animation.
    setTimeout(function() {{
...
"""
    html_str = html_str.replace("</body>", inject + "</body>", 1)
    _components.html(html_str, height=height + 16, scrolling=False)
...
if page == "🏠  Home":
    ...
    if hist:
        try:
            ...
            _show_plotly(fig, height=320, key="fig_home_history")
        except ImportError:
        ...
        if not hist:
            ...
            if _PLOTLY:
                ...
                fig_loss.update_layout(
                    ...
                    margin=dict(l=10, r=10, t=40, b=10),
                    height=300,
                    uirevision=f"loss_{_sel_run}",
                )

                col_a, col_b = st.columns(2)
                with col_a:
                    _show_plotly(fig_acc, height=300, key=f"fig_acc_{_sel_run}")
                with col_b:
                    _show_plotly(fig_loss, height=300, key=f"fig_loss_{_sel_run}")
            else:
                st.line_chart(
            ...
            if _PLOTLY:
                ...
                fig_gap.update_layout(
                    ...
                    margin=dict(l=10, r=10, t=40, b=10),
                    height=250,
                    uirevision=f"gap_{_sel_run}",
                )
                _show_plotly(fig_gap, height=250, key=f"fig_gap_{_sel_run}")
        ...
        if not CM_PATH.exists():
            ...
            if _PLOTLY:
                ...
                fig_cm.update_layout(
                    ...
                    height=420,
                    uirevision=f"cm_{_sel_run}_{use_norm}",
                )
                _show_plotly(fig_cm, height=420, key=f"fig_cm_{_sel_run}_{use_norm}")
            else:
        ...
        if not CM_PATH.exists():
            ...
            if _PLOTLY:
                ...
                radar_fig.update_layout(
                    ...
                    margin=dict(l=10, r=10, t=50, b=10),
                    height=380,
                    uirevision=f"radar_{_sel_run}",
                )
                _show_plotly(radar_fig, height=380, key=f"fig_radar_{_sel_run}")
                ...
                fig_f1.update_layout(
                    ...
                    margin=dict(l=10, r=10, t=40, b=10),
                    height=280,
                    uirevision=f"f1_{_sel_run}",
                )
                _show_plotly(fig_f1, height=280, key=f"fig_f1_{_sel_run}")
        ...
        if not TSNE_PATH.exists():
            ...
            if _PLOTLY:
                ...
                fig_tsne.update_layout(
                    ...
                    margin=dict(l=10, r=10, t=50, b=10),
                    height=520,
                    uirevision=f"tsne_{_sel_run}",
                )
                _show_plotly(fig_tsne, height=520, key=f"fig_tsne_{_sel_run}")
                ...
                fig_dist.update_layout(
                    ...
                    height=240,
                    uirevision=f"dist_{_sel_run}",
                )
                _show_plotly(fig_dist, height=240, key=f"fig_dist_{_sel_run}")
            else:
                st.scatter_chart(
            ...
            with st.expander(
                ...
                try:
                    ...
                    fig2.update_layout(
                        ...
                        margin=dict(l=10, r=10, t=20, b=10),
                        height=280,
                        uirevision=f"infer_{fname}",
                    )
                    _show_plotly(fig2, height=280, key=f"fig_infer_{fname}")
                except ImportError:
