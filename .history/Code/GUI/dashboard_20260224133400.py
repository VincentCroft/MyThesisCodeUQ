...
def render_plotly(fig, height: int = 400, key: str = "") -> None:
    ...
    # If the user explicitly set a right margin (e.g. for radar/tsne), respect it.
    # Otherwise, let Plotly auto-size it.
    _prev_r = None
    try:
        _prev_r = lay.margin.r
    except Exception:
        pass
    ...
    if _prev_r is not None:
        lay.margin.r = _prev_r
    else:
        lay.margin.r = 160  # Default to 160 to ensure legend is never clipped

    # 3. Let JS measure the real container width and pass it explicitly.
    #    autosize=False ensures Plotly respects our margin.r exactly.
    lay.autosize = False
...
(function(){{
  ...
  /* Initial plot with explicit width — autosize:false guarantees margin.r is respected */
