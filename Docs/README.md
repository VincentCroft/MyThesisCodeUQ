...
│   ├── Utils/
│   │   ├── __init__.py
│   │   └── analysis.py               # 分析工具：混淆矩阵、t-SNE、分类报告
│   ├── GUI/
...
│           ├── confusion_matrix.json # 混淆矩阵及逐类指标（训练后生成）
│           └── tsne_embeddings.json  # t-SNE 特征嵌入坐标（训练后生成）
...
| `confusion_matrix.json` | 混淆矩阵（原始 + 归一化）及逐类 P/R/F1 指标 |
| `tsne_embeddings.json` | 倒数第二层特征的 t-SNE 二维嵌入坐标 |
...
> 需要先完成至少一次训练，才能看到分析数据。

页面包含四个子标签：

##### 📉 Loss & Accuracy 曲线

- 双面板 Plotly 折线图：左侧 Accuracy（训练/验证），右侧 Loss（训练/验证）
- 最佳 epoch 以竖线标注
- 过拟合差值图：展示 `Train Acc − Val Acc` 随轮次的变化，并以 5% / 10% 阈值线标注拟合质量

##### 🔲 Confusion Matrix — 混淆矩阵

- 支持切换**原始计数**和**行归一化（百分比）**两种视图
- Plotly 热力图，支持悬停查看数值
- 附逐类 Precision / Recall / F1 / 支持数量表格

##### 📋 Classification Report — 分类报告

- **雷达图**：对比各类别的 Precision、Recall、F1（多边形填充，便于视觉对比）
- **F1 Bar Chart**：各类别 F1 分数柱状图
- **汇总表格**：含宏平均（Macro Avg）行和总体准确率行

##### 🔵 Feature Cluster (t-SNE) — 特征聚类

- 对模型倒数第二层的特征向量做 **t-SNE 降维**（最多 3,000 个验证样本）
- 散点图按故障类别着色，可直观判断模型学到的类间判别能力
- 附各类别样本数量柱状图
...
所有文件路径集中定义在 **`Code/configs/paths.py`**，需要调整目录结构时只需修改此文件，无需逐一搜索代码。

```python
# Code/configs/paths.py（节选）

# 项目根目录（自动推算，无需修改）
CODE_DIR    = Path(__file__).resolve().parent.parent   # .../Thesis/Code
THESIS_ROOT = CODE_DIR.parent                          # .../Thesis

# ── 数据目录 ──────────────────────────────────────────────
RAW_DATA_ROOT       = THESIS_ROOT / "DataSet"          # 原始采集数据
PROCESSED_DATA_ROOT = THESIS_ROOT / "ProcessedData"    # 预处理后数据（训练来源）

# ── 检查点输出 ────────────────────────────────────────────
CKPT_DIR              = CODE_DIR / "logs" / "checkpoints"
BEST_MODEL_PT         = CKPT_DIR / "best_model.pt"
NORMALIZER_NPZ        = CKPT_DIR / "normalizer.npz"
TRAINING_HISTORY_JSON = CKPT_DIR / "training_history.json"
CLASS_ACC_JSON        = CKPT_DIR / "class_accuracy.json"
MODEL_META_JSON       = CKPT_DIR / "model_meta.json"
CONFUSION_MATRIX_JSON = CKPT_DIR / "confusion_matrix.json"
TSNE_EMBEDDINGS_JSON  = CKPT_DIR / "tsne_embeddings.json"

# ── 配置文件 ──────────────────────────────────────────────
TRAIN_CONFIG_YAML = CODE_DIR / "configs" / "train_config.yaml"
```

> **迁移项目时只需修改 `THESIS_ROOT` 一行**（或把 `paths.py` 放到新位置），其余路径全部自动更新。
...
> **注意**：检查点输出路径现已由 `configs/paths.py` 统一管理，`train_config.yaml` 中的 `checkpoint_dir` 字段已不再使用。如需更改输出目录，请修改 `paths.py` 中的 `CKPT_DIR`。
...
文档版本：v1.1 | 协议版本：PMU Protocol v1.2 | 更新日期：2026-02-23
